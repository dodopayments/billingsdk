...
const portalVariants = cva(
  "flex flex-col md:flex-row min-h-[600px] w-full rounded-xl border overflow-hidden",
  {
    variants: {
      theme: {
...
const portalButtonVariants = cva(
  "relative inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 overflow-hidden active:scale-[0.98]",
  {
    variants: {
      variant: {
        premium:
          "bg-white/10 backdrop-blur-md border border-white/20 text-foreground shadow-sm hover:bg-white/20 hover:shadow-md hover:scale-[1.02]",
        outline:
...
export function CustomerPortal({
  ...
  return (
    <div className={cn(portalVariants({ theme }), className)}>
      {/* Mobile Header */}
      <div className="bg-background/80 flex items-center justify-between border-b p-4 backdrop-blur-sm md:hidden">
      ...
      <aside
        className={cn(
          "flex flex-col border-r transition-all duration-300 md:w-64",
          isMobileMenuOpen
            ? "bg-background fixed inset-0 z-50 flex md:relative md:bg-transparent"
            : "hidden md:flex",
        ...
        <nav className="flex-1 space-y-1 py-4">
          {menuItems.map((item) => (
            <button
              ...
              className={cn(
                sidebarItemVariants({ active: activeTab === item.id }),
                "group",
              )}
            >
              ...
              {activeTab === item.id && (
                <ChevronRight className="animate-in slide-in-from-left-2 ml-auto size-4 duration-300" />
              )}
            </button>
          ))}
        </nav>
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="flex hidden h-16 items-center border-b px-8 md:flex">
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8">
          <div className="animate-in fade-in slide-in-from-bottom-2 mx-auto max-w-4xl space-y-8 duration-500">
            {activeTab === "overview" &&
              (children?.overview || (
                <OverviewPlaceholder theme={theme ?? "minimal"} />
              ))}
            {activeTab === "subscription" &&
              (children?.subscription || (
                <ContentPlaceholder title="Subscription Management" />
              ))}
            {activeTab === "payment" &&
              (children?.payment || (
                <ContentPlaceholder title="Payment Methods" />
              ))}
            {activeTab === "history" &&
              (children?.history || (
                <ContentPlaceholder title="Invoice History" />
              ))}
            {activeTab === "profile" &&
              (children?.profile || (
                <ContentPlaceholder title="Profile Settings" />
              ))}
            {activeTab === "notifications" &&
              (children?.notifications || (
                <ContentPlaceholder title="Notification Preferences" />
              ))}
            {activeTab === "security" &&
              (children?.security || (
                <ContentPlaceholder title="Security Settings" />
              ))}
          </div>
        </div>
      </main>
...
function OverviewPlaceholder({ theme }: { theme: "minimal" | "classic" }) {
  return (
    <div className="grid gap-6">
      ...
      <Card
        className={cn(
          "flex h-64 animate-pulse items-center justify-center border-dashed",
          theme === "classic" ? "bg-card/20" : "bg-muted/10",
        )}
      >
        <CardContent>
          <div className="text-muted-foreground">
            Loading dynamic dashboard content...
          </div>
        </CardContent>
      </Card>
    </div>
...
function ContentPlaceholder({ title }: { title: string }) {
  return (
    <Card className="bg-muted/10 flex h-64 flex-col items-center justify-center border-dashed text-center">
      <CardHeader>
        <div className="bg-muted/50 mx-auto mb-4 flex size-16 items-center justify-center rounded-full">
          <LayoutDashboard className="text-muted-foreground/50 size-8" />
        </div>
