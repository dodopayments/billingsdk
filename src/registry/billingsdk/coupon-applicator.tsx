...
/**
 * Interactive coupon input and applicator UI.
 *
 * Renders a coupon input with local validation/UI state and delegates server-side verification to `onApply`.
 * Shows loading, success, and error states, computes and displays discount breakdown when a coupon is applied,
 * and supports auto-apply on blur, keyboard submit (Enter), and removal of an applied coupon.
 *
 * @param onApply - Callback invoked with the trimmed, uppercased coupon code; should return a Promise resolving to a ValidationResult indicating validity, optional discount, messages, and computed totals.
 * @param currentPrice - Current price (pre-discount) used to compute savings and new total when a discount is returned.
 * @param className - Optional container className for custom styling.
 * @param theme - Visual variant, either `'classic'` (default) or `'minimal'` which alters borders/backgrounds.
 * @param autoApply - If true, triggers `onApply` automatically on input blur when there is a non-empty code (default: false).
 * @param onRemove - Optional callback invoked when an applied coupon is removed.
 * @returns A JSX element containing the coupon applicator UI.
 */
