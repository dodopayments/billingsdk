...
function TabNavigation({ activeTab, onTabChange }: TabNavigationProps) {
  return (
    <div className="flex flex-col sm:flex-row gap-1 rounded-lg bg-muted p-1">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => {
          ...
          className={`w-full sm:flex-1 text-start sm:text-center rounded-md px-3 py-1 sm:py-2 text-sm font-medium transition-colors cursor-pointer ${
            activeTab === tab.id
...
export function BillingSettings({
  ...
  return (
    <Card className={`mx-auto md:min-w-xl max-w-2xl overflow-hidden ${className || ''}`}>
      <CardHeader className="space-y-2 sm:space-y-4 px-4 sm:px-6 sm:pb-6">
