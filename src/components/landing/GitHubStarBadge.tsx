...
export default function GitHubStarBadge() {
  ...
  return (
    <Link
      ...
      className="inline-flex h-9 items-center rounded-md border bg-background text-xs leading-none font-normal text-white shadow-sm backdrop-blur-md transition-colors"
      aria-label="View GitHub repository and star"
    >
      {/* Left: GitHub icon */}
      <span className="flex h-full items-center rounded-l-md px-1.5">
        <FaGithub className="h-5 w-5" />
      </span>
      {/* Middle: number */}
      <span className="flex h-full w-10 items-center justify-center border-l px-1 pl-2 text-sm font-normal">
      ...
      <span className="flex h-full items-center pr-2 pl-1">
        <AnimatePresence mode="popLayout" initial={false}>
          {display && (
            <motion.span
              key={display.kind}
