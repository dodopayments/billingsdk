  ...
  "relative overflow-hidden transition-all duration-300",
  {
    variants: {
      theme: {
        ...
        classic:
          "bg-card/50 backdrop-blur-sm border-border/50 shadow-sm hover:shadow-xl",
      },
    },
    defaultVariants: {
      theme: "minimal",
  ...
  return (
    ...
    >
      <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
        <CardTitle className="text-muted-foreground flex min-h-[2.5rem] items-start text-sm leading-tight font-medium">
          {title}
        </CardTitle>
        {trend !== undefined && (
          <div
            className={cn(trendVariants({ type: trendType }), "mt-1 shrink-0")}
          >
      ...
      <CardContent className="flex flex-1 flex-col justify-between pt-2">
        <div>
          <div className="flex items-baseline text-3xl font-bold tracking-tight tabular-nums">
            {isCurrency && (
          ...
          {description && (
            <p className="text-muted-foreground mt-2 flex min-h-[2.5rem] items-start text-xs leading-relaxed">
              {description}
            </p>
          )}
        </div>
        ...
        {theme === "classic" && (
          <div className="from-primary/5 absolute inset-0 -z-10 bg-gradient-to-br via-transparent to-transparent opacity-50" />
        )}
      </CardContent>
    </Card>
  );
