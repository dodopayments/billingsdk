...
- **Framework**: Next.js (with App Router) or Express.js
...
- 🔧 **Features**: REST API routes, middleware support, Zod validation, modular routing
...
Initialize a Express.js integration with Dodo Payments using the CLI:
...
This command creates `src/lib/dodopayments.ts` and `src/routes/dodopayments/*`, `.env.example` and install dependencies like `zod`, `express`, `dodopayments`, `standardwebhooks`, `dotenv`.


### 2) Add the main Router

```ts
import express from "express";
import dotenv from "dotenv";
import dodopaymentsRoutes from "./src/routes/dodopayments/index.ts"; 
...
app.get("/", (_req, res) => {
  res.json({ message: "🚀 Express + DodoPayments is running!" });
});


app.use("/dodopayments", dodopaymentsRoutes);
...
### 4) Webhook
- Configure your provider to call /api/webhook (Next.js) or /routes/  dodopayments/webhook (Express.js)
- `DODO_PAYMENTS_WEBHOOK_KEY` to securely verify incoming webhook requests.
