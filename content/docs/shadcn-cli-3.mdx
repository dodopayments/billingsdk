...
Add components directly using the trusted namespace:
...
<Tabs items={['npx', 'pnpm', 'yarn', 'bun']} defaultValue="npx" groupId="installation-tabs">
  ...
  <Tab value="yarn">
    ```bash
    yarn dlx shadcn@latest add @billingsdk/[component-name]
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bunx shadcn@latest add @billingsdk/[component-name]
    ```
  </Tab>
</Tabs>
...
<Tabs items={['npx', 'pnpm', 'yarn', 'bun']} defaultValue="npx" groupId="billingsdk-cli-tabs">
  ...
  <Tab value="yarn">
    ```bash
    yarn dlx @billingsdk/cli add <component-name>
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bunx @billingsdk/cli add <component-name>
    ```
  </Tab>
</Tabs>

> **Note**: Replace `<component-name>` with the actual component name you want to add, such as `pricing-table-one` or `payment-method-manager`.

## Namespaced Registry Setup

ShadCN CLI 3.0 introduces namespaced registries, allowing you to organize and access component collections more efficiently. This feature is particularly useful for managing multiple component libraries.

### Configuration

To set up the Billing SDK namespace, update your `components.json` file:
...
This configuration maps the `@billingsdk` namespace to the Billing SDK component registry.

### Usage

Once configured, you can add components using the simplified namespace syntax:
...
# Add a payment method manager
npx shadcn@latest add @billingsdk/payment-method-manager

# Add a subscription management component
npx shadcn@latest add @billingsdk/subscription-management
```

> **Tip**: The namespaced approach eliminates the need to remember full URLs and provides better autocomplete support in compatible editors.
